...
export function PurchaseCreateModal({ isOpen, onClose, onSuccess }: PurchaseCreateModalProps) {
  ...
  const [items, setItems] = useState([{ productId: "", quantity: 1, purchaseRate: 0 }]);
  ...
  const addItem = () => setItems([...items, { productId: "", quantity: 1, purchaseRate: 0 }]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setItems([{ productId: "", quantity: 1, purchaseRate: 0 }]);
    } catch (error) {
      console.error(error);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[850px] max-h-[92vh] overflow-y-auto bg-white rounded-2xl p-6">
        <form onSubmit={handleSubmit}>
          <DialogHeader className="border-b pb-3 mb-4">
            <DialogTitle className="text-xl font-bold text-slate-900">New Purchase & Stock Receiving</DialogTitle>
            <p className="text-xs text-slate-500">Record supplier purchase bill. Received stock will immediately enter FIFO inventory layers.</p>
          </DialogHeader>
          ...
          <div className="grid gap-4">
            ...
            <div className="mt-2 border rounded-xl p-4 bg-slate-50/50 space-y-3">
              <div className="flex justify-between items-center">
                <h4 className="text-sm font-bold text-slate-900">Purchase Line Items (Stock Items)</h4>
              ...
              {items.map((item, index) => {
                ...
                return (
                  <div key={index} className="flex flex-wrap gap-2 items-end bg-white p-3 rounded-lg border">
                      ...
                      >
                        ...
                        {products.map((p) => (
                          <option key={p.id} value={p.id}>
                            {p.name} ({p.sku}) - {p.unit?.abbreviation || ""}
                          </option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div className="w-28 space-y-1">
                      <Label className="text-xs font-medium">Qty ({prod?.unit?.abbreviation || "Units"}) *</Label>
                      <Input
                        type="number"
                        min="0.001"
                        step="any"
                    ...
                    <div className="w-32 space-y-1">
                      <Label className="text-xs font-medium">Purchase Rate (PKR) *</Label>
                      <Input
                        type="number"
                        min="0.01"
                        step="any"
                    ...
                    <div className="w-36 space-y-1">
                      <Label className="text-xs font-medium">Line Subtotal</Label>
                      <Input
                        readOnly
                        value={formatCurrency(item.quantity * item.purchaseRate)}
                        className="bg-slate-50 text-xs font-bold text-slate-900"
                      />
                    </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={handleChange}
                    placeholder="e.g. 100 kg milk powder in 20kg bags"
          ...
          <DialogFooter className="mt-6 border-t pt-4">
            ...
            <Button type="submit" disabled={loading} className="bg-blue-600 text-white font-semibold shadow-sm">
              {loading ? "Processing..." : "Save Purchase & Add Stock"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
