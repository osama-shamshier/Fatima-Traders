...
export async function POST(request: Request) {
  try {
    ...
    if (!buyerId || !amountNum || amountNum <= 0 || !paymentMethod) {
      return NextResponse.json({ error: "Buyer, valid positive amount, and payment method are required" }, { status: 400 });
    }
    ...
    const payment = await prisma.$transaction(async (tx) => {
      ...
      if (saleId) {
        ...
        for (const sale of pendingSales) {
          ...
          const currentOutstanding = Number(sale.outstandingAmount || Math.max(0, totalAmount - currentPaid));
